...
export class AppPageController {
  ...
  async get(@Param('path') path: string, @Param('slug') slug: string) {
    ...
    try {
      ...
      Logger.log('AppPageController, Success loading content', {
        path,
        slug,
        data,
      });
      ...
      Logger.log('AppPageController, Error loading content', {
        path,
        slug,
        error,
      });
      throw new Error(
        error instanceof Error ? error.message : 'Error while loading content',
      );
    }
  }
}
