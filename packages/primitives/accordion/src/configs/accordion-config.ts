...
export interface NgxPrimerAccordionConfig {
  ...
  collapsible: boolean;
  orientation: NgxPrimerAccordionOrientation | keyof typeof NgxPrimerAccordionOrientation;
  theme: {
    builtIn: boolean
  }
}
...
export function provideAccordionConfig(config: Partial<NgxPrimerAccordionConfig>) : Provider {
  return [
    {
...
export function injectAccordionConfig(): NgxPrimerAccordionConfig {
  return inject(NgxPrimerAccordionConfigToken, { 
    optional: true,
    self: true,
    host: true,
  }) ?? defaultAccordionConfig()
